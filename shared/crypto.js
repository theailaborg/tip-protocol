...
//
// The implementation now lives in tip-content-fingerprint's canonical module
// (single source shared by node, extension, and web) and is frozen there with
// golden fixtures; changing it is a protocol migration (CNA-3), never an
// in-place edit. Deep import of the dependency-free leaf module per this
// repo's convention (see node/src/perceptual/ingest.js): canonical/index.js
// would pull in @noble/hashes (ESM-only), which Jest's CJS runtime cannot
// require.
const { tipNormalize } = require('tip-content-fingerprint/src/canonical/normalize');
