...
CREATE TABLE IF NOT EXISTS `prescan_jobs` (`job_id` varchar(128), `tip_ctid` varchar(512) not null, `payload` blob not null, `status` varchar(16) not null, `claimed_at` bigint null, `claimed_by` varchar(128) null, `retries` integer not null default '0', `last_error` text null, `created_at` bigint not null, `completed_at` bigint null, `retry_after` bigint not null default '0', `classifier_job_id` varchar(128) null, `classifier_job_at` bigint null, `classifier_polls` integer not null default '0', primary key (`job_id`));
...
CREATE TABLE IF NOT EXISTS `upload_sessions` (`session_id` varchar(64), `upload_id` varchar(256) not null, `s3_key` varchar(512) not null, `content_hash` varchar(64) not null, `mime` varchar(128) not null, `size` bigint not null, `signer_tip_id` varchar(512) not null, `timestamp` bigint not null, `signature` text not null, `parts_json` text not null default '[]', `completed_size` bigint not null default '0', `created_at` bigint not null, `expires_at` bigint not null, `state` varchar(16) not null default 'uploading', `result_json` text null, `checksum_algorithm` varchar(8) null, primary key (`session_id`));
