...
//  • All tables are created via Knex migrations (src/db/migrations/). migrate()
//    calls knex.migrate.latest() which is idempotent — safe to call on every boot.
...
class KnexAdapter {
  ...
  async _hydrate() {
    ...
    // pattern as the content table — tip_ctid used because PostgreSQL
    // reserves "ctid" as a system column).
