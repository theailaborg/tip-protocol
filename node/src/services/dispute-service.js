...
function createDisputeService({ dag, scoring, config, submitTx, submitBatch, disputeDetailsService }) {
    ...
    try {
      {
        ...
        if (!r.valid) throw { status: r.error.status, error: r.error.message, ...(r.error.code ? { code: r.error.code } : {}) };
      }
      ...
      const disputeTx = withTxId({
        tx_type: TX_TYPES.CONTENT_DISPUTED, timestamp: nowMs(),
        data: {
      ...
      const classifierTx = nodeSignedAuto({
        tx_type: TX_TYPES.AI_CLASSIFIER_RESULT, timestamp: nowMs(),
      ...
      for (const jurorTipId of jury.jurors) {
        const summonsTx = nodeSignedAuto({
          tx_type: TX_TYPES.JURY_SUMMONS, timestamp: nowMs(),
  ...
  function juryCommit(ctid, body) {
    ...
    const commitTx = withTxId({ tx_type: TX_TYPES.JURY_VOTE_COMMIT, timestamp: nowMs(), data: { ctid, juror_tip_id, commitment, is_appeal: false }, signature }, dag);
    submitTx(commitTx);
  ...
  function juryReveal(ctid, body) {
    ...
    const revealTx = withTxId({ tx_type: TX_TYPES.JURY_VOTE_REVEAL, timestamp: nowMs(), data: { ctid, juror_tip_id, vote, salt, confirmed_origin: vote === VOTE.MISMATCH ? confirmed_origin : null, is_appeal: false }, signature }, dag);
    submitTx(revealTx);
  ...
  function fileAppeal(ctid, body) {
    ...
    const appealTx = withTxId({ tx_type: TX_TYPES.APPEAL_FILED, timestamp: nowMs(), data: { ctid, appellant_tip_id, author_tip_id: authorTipId, disputer_tip_id: disputerTipId, stage2_verdict: adjTxs[0].data.verdict, stake: APPEAL.APPELLANT_STAKE }, signature }, dag);
  ...
  function appealCommit(ctid, body) {
    ...
    const commitTx = withTxId({ tx_type: TX_TYPES.JURY_VOTE_COMMIT, timestamp: nowMs(), data: { ctid, juror_tip_id, commitment, is_appeal: true }, signature }, dag);
    submitTx(commitTx);
    return { success: true, tx_id: commitTx.tx_id, confirmation: "proposed" };
  }
  ...
  function appealReveal(ctid, body) {
    ...
    const revealTx = withTxId({ tx_type: TX_TYPES.JURY_VOTE_REVEAL, timestamp: nowMs(), data: { ctid, juror_tip_id, vote, salt, confirmed_origin: vote === VOTE.MISMATCH ? confirmed_origin : null, is_appeal: true }, signature }, dag);
    submitTx(revealTx);
