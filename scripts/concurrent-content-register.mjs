...
// Pick any founding identity backup (filename changes on every seed:fresh).
const _backupDir = path.join(ROOT, 'genesis-data', 'backups');
const _backupFile = fs.readdirSync(_backupDir).find(n => n.startsWith('tip-id-') && n.endsWith('.tip.json'));
