...
2. `PUT` each part's bytes to its `url` and keep the returned `ETag`. Part URLs are valid until `url_expires_at` (2 h by default, node-configurable); refresh before that, or after a `403`, from `GET /v1/media/upload-status/:session_id`, which returns `{ state: "uploading", part_count, uploaded_parts, missing_parts, parts: [{ part_number, url, url_expires_at }], expires_at }`. Send `Content-MD5` on each part PUT and S3 rejects a corrupted part on receipt, independent of ETag semantics.
